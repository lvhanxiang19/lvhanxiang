...
def main(args):
    ...
    #数据集路径
    data = sorted(glob.glob(os.path.join(args.data_path, "img", "*.png")))
    label = sorted(glob.glob(os.path.join(args.data_path, "label", "*.png")))
    ...
    for epoch in range(args.epochs):
        ...
        if best_dice[0].sum(dim=0) < val_dice[0].sum(dim=0):
            torch.save(model.state_dict(), r"C:\Users\32053\Desktop\unet_cells\weights\best_model_xibao.pth")
            best_dice = val_dice
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str,default=r"C:\Users\32053\Desktop\Dataset_BUSI_with_GT")
